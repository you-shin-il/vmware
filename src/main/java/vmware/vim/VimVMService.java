...
public class VimVMService {
    ...
    public void cloneVMTask() throws RuntimeFaultFaultMsg, InvalidPropertyFaultMsg {
        List<ObjectContent> template = getTemplate();
        ...
        VirtualMachineConfigInfo virtualMachineConfigInfo = (VirtualMachineConfigInfo)
                propSet.stream().filter(dynamicProperty -> "config".equals(dynamicProperty.getName())).findFirst().map(x -> x.getVal()).get();

        VirtualMachineConfigSpec vmcs = newVirtualMachineConfigSpec();
        vmcs.setCpuAllocation(virtualMachineConfigInfo.getCpuAllocation());
        vmcs.setMemoryAllocation(virtualMachineConfigInfo.getMemoryAllocation());
    ...
    public List<ObjectContent> getTemplateSpec(ObjectContent objectContent) throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg {
    ...
    public List<ObjectContent> getTemplate() throws RuntimeFaultFaultMsg, InvalidPropertyFaultMsg {
...
/*        TraversalSpec ts = new TraversalSpec();
    ...
    public List<ObjectContent> isTemplateFilter(List<ObjectContent> objs) {
        List<ObjectContent> objectsFilter = new ArrayList<>();

        for (ObjectContent obj : objs) {
            for (DynamicProperty dynamic : obj.getPropSet()) {
                if ("config".equals(dynamic.getName())) {
                    VirtualMachineConfigInfo vmConfigInfo = (VirtualMachineConfigInfo) dynamic.getVal();
                    if (vmConfigInfo.isTemplate()) {
                        objectsFilter.add(obj);
                    }
                }
            }
        }
